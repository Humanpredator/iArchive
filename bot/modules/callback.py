...
from bot.helper.ig_utils.ig_down import check_username, download_posts, download_all_posts, download_igtv, \
    fetch_followers, fetch_following, download_highlights, download_feed, download_saved, download_tagged, \
    download_following_stories, download_stories
...
def cb_handler(update, context):
    ...
    if query_type == "PPIC":
            ...
            [
                [
                    InlineKeyboardButton(
                        "Picture Posts", callback_data=f"PIC#{user_id}"
                    ),
                    InlineKeyboardButton(
                        "Video Posts", callback_data=f"VID#{user_id}"
                    ),
                ],
                [
                    InlineKeyboardButton(
                    ...
                    InlineKeyboardButton(
                        "ALL Posts", callback_data=f"ALLPOST#{user_id}"
                    ),
                ],
            ]
        )
        ...
        bot.send_message(
            chat_id=query.message.chat.id, text="Process Cancelled by User")

    elif query_type == "FOLLOWER":
        query.answer()

        msg = bot.send_message(
            query.message.chat.id, f"Fetching Followers List of {full_name}")
        ...
        msg = bot.send_message(
            query.message.chat.id, f"Fetching Following list of {full_name}")
        ...
        msg = bot.send_message(
            query.message.chat.id, f"Downloading Highlights of {full_name}")
        ...
        msg = bot.send_message(
            query.message.chat.id, f"Downloading Feed of {full_name}")
        download_feed(msg)
        ...
        msg = bot.send_message(
            query.message.chat.id, f"Downloading Saved of {full_name}")
        ...
        msg = bot.send_message(
            query.message.chat.id, f"Something went wrong. Please try again later."
        )
